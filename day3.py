...
class PartNumber(object):
    ...
    def grabpn(self, r: int, c: int) -> None | int:
        return int(
            re.findall("\d+", self.lines[r][: c + 1])[-1]
            + re.findall("\d+", self.lines[r][c:])[0][1:]
        )
...
def solve(lines: list[str], match: str):
    return [
        {
            int(
                re.findall("\d+", lines[r][: c + 1])[-1]
                + re.findall("\d+", lines[r][c:])[0][1:]
            )
            for r in range(row - 1, row + 2)
            for c in range(col - 1, col + 2)
            if (r >= 0 and r < len(lines) and c >= 0 and c < len(lines[r]))
            and lines[r][c].isdigit()
        }
        for row, col in {
            (r, c)
